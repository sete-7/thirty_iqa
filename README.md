...
├── scorers/
│   ├── expert_scorers.py     # 串行专家打分统筹逻辑
│   └── qinsight_qprobe.py    # Q-Insight+ (主动观看/找瑕疵区域) + Q-Probe (局部评分)
├── cot/
...
from features.feature_extractor import process_features_with_checkpointing
...
```python
from scorers.expert_scorers import process_scoring_with_checkpointing
...
|---|---|
| UniPercept | 感知质量 |
| Q-Insight+ / Q-Probe | 主动观看：全局扫描找瑕疵区域 + 细节局部评分 |
| HPSv3 | 语义 / 图文匹配 |
| SpatialScore | 空间布局理解 |
...
```python
from data.data_filter import filter_data_middle_60
...
```python
from cot.cot_generator import process_cot_generation
...
```python
from calibration.mos_calibrator import calibrate_mos_from_votes
...
```python
from packager.dataset_packager import package_final_dataset
...
`features.feature_extractor`、`scorers.expert_scorers` 和 `cot.cot_generator` 均支持 **每 100 张图片自动保存进度**。如果运行中断，重新运行同一命令即可——已处理的图片会自动跳过。
